...
object ShaderUtil {
    ...
    fun generateDrawOrderBuffer(): ShortBuffer {
        val drawBuffer = ByteBuffer.allocateDirect(drawOrder.size * 2) //一个short有4个字节
                .order(ByteOrder.nativeOrder())
                .asShortBuffer()
                .put(drawOrder)
     ...
     */
    fun useGL(program: Int, position: Int, positionBuffer: FloatBuffer, coordinate: Int, coordinateBuffer: FloatBuffer) {
        //启用gl
     ...
     * @param height 图片高
     */
    fun bindTexture(offset: Int, id: Int, texture: Int, buffer: ByteBuffer, width: Int, height: Int) {
        //激活指定纹理
        GLES20.glActiveTexture(GLES20.GL_TEXTURE0 + offset)
        //绑定纹理索引
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D + offset, id)
    ...
    fun drawGL(drawBuffer: ShortBuffer, position: Int, coordinate: Int) {
        GLES20.glDrawElements(GLES20.GL_TRIANGLE_STRIP, drawOrder.size, GLES20.GL_UNSIGNED_SHORT, drawBuffer)
     ...
     * @param height fbo高
     */
    fun generateFrameBufferObject(frameBuffer: IntArray, frameTexture: IntArray, width: Int, height: Int) {
        //创建FBO
     ...
     * 生成指定数量的FBO
     */
    fun generateFrameBufferObject(frameBuffer: IntArray, frameTexture: IntArray, width: Int, height: Int,size:Int){
